...
class AddOnsTable extends WP_List_Table {
    ...
    function column_name($item){
        ...
        //Build row actions
        $actions = array(
            'settings'      => sprintf('<a href="?page=%s&action=%s&addon=%s">Settings</a>',$_REQUEST['page'],'settings',$item['ID']),
            'deactivate'      => sprintf('<a href="?page=%s&action=%s&addon=%s">Deactivate</a>',$_REQUEST['page'],'deactivate',$item['ID']),
            'delete'    => sprintf('<a href="?page=%s&action=%s&addon=%s">Delete</a>',$_REQUEST['page'],'delete',$item['ID']),
        );
        $editurl = sprintf('?page=%s&action=%s&addon=%s',$_REQUEST['page'],'view',$item['ID']);
        //Return the title contents
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
            ...
            foreach($terms AS $term) {
                if(wp_delete_term( $term, 'wp_aff_categories' )) {
                    $del ++;
                } else {
                    $err ++;   
                }
            ...
            if($del > 0) {
                echo '<div id="message" class="updated below-h2"><p>'.$del.' Categories Deleted.</p></div>';
            }
            if($err > 0 ) {
                echo '<div id="message" class="error below-h2"><p>'.$err.' Categories Not Found.</p></div>';
            }
            if( $del == 0 && $err == 0 ) { 
    ...
    function prepare_items() {
        ...
        $terms = get_terms(array( 'wp_aff_categories' ), array( 'hide_empty' => false ) );
        $data = array();
        $data[] = array (
            'ID'    => 1,
            'name'  => 'Affiliate Window API',
            'desc'  => 'Use products from the Affiliate Window Product Search API'
        );
